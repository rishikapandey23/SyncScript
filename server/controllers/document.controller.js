...
const getDocumentById = async (req, res) => {
  ...
  try {
    ...
    if(document.createdBy.toString() !== userId && !document.sharedWith.some(item => item._id.toString() === userId)) {
        return res.status(403).json({ error: "You are not authorized to view this document" });
    }
    document = document.toObject();
    if(template === "resume") {
      res.status(200).json({...document, requestUserId: userId, content: resume});
      return;
    }
